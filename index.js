...
//   console.log("connect looooooooo", uri)
...
async function run() {
  try {
    ...
    // input a file backend a pathanor  jonno
    ...
    app.get("/users/:email", async (req, res) => {
      ...
      const user = await usersConnection.findOne(query);
      let isAdmin = false;
      if (user?.role === "admin") {
        isAdmin = true;
      }
    ...
    app.put("/users", async (req, res) => {
      ...
      const result = await usersConnection.updateOne(
        filter,
        updateDoc,
        options
      );
    ...
    app.put("/users/admin", verifyToken, async (req, res) => {
      ...
      if (requester) {
        const requesterAccount = await usersConnection.findOne({
          email: requester,
        });
        ...
        if (requesterAccount.role === "admin") {
          ...
          const result = await usersConnection.updateOne(filter, updateDoc);
          res.json(result);
        }
      } else {
        res.status(403).json({ massage: "you don not have access" });
      }
    });
